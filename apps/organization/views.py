...
class OrgView(View):
    ...
    def get(self, request):
        ...
        # 精确显示具体某个城市的某个类型的所有机构
        if category and city_id:
            all_orgs = all_orgs.filter(Q(category=category) & Q(city_id=int(city_id)))
...
class OrgHomeView(View):
    """
    机构首页：显示机构的基本信息
    课程：机构的前三个课程，点击更多时，传递该机构道ID查出所有课程，跳转到课程列表页面
    讲师：机构的前一个课程，点击更多时，传递该机构道ID查出所有讲师，跳转到讲师列表页面
    简介：机构到简介，点击更多进入详情页
    ...
    def get(self, request, org_id):
        ...
        # 用户进入后，点击数加1，这个写法在访问量比较大的时候需要优化，数据库会有瓶颈
